...
const ReviewItem = ({ review, onRemoved }) => {
  ...
  const onRemove = async () => {
    ...
    if (response) onRemoved(review.reviewId);
  ...
  return (
    ...
    >
      <Stack direction="row" spacing={2}>
        {/* avatar */}
        <TextAvatar text={review.Users?.username} />
        {/* avatar */}
...
const MediaReview = ({ reviews, media, mediaType }) => {
  ...
  const onRemoved = (id) => {
    const existsInList = listReviews.some((e) => e.reviewId === id);

    if (existsInList) {
      const newListReviews = listReviews.filter((e) => e.reviewId !== id);

      setListReviews(newListReviews);

      setFilteredReviews(newListReviews.slice(0, page * skip));
    } else {
      setFilteredReviews(filteredReviews.filter((e) => e.reviewId !== id));
    }
